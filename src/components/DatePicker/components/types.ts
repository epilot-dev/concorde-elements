...
export type DatePickerTextFieldProps = Omit<InputProps, 'id'> & {
  ...
  /**
   * Called when the validity of the typed input changes. Emits `false` when
   * the entered text is a complete but invalid/non-existent date (e.g. 30.02.),
   * and `true` once it becomes a valid date or is cleared. Lets consumers
   * surface an error and block submission instead of silently coercing to today.
   */
