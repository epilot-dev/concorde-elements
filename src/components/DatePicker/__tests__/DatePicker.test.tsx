...
describe('DatePicker', () => {
  ...
  describe('typed input validation', () => {
    test('resets to null and flags invalid for a non-existent date', async () => {
      const user = userEvent.setup()
      ...
      // 30.02.2025 is a complete but non-existent date
      await user.keyboard('30022025')

      // It must be flagged invalid and reset to null, not coerced to today
      expect(onValidityChange).toHaveBeenLastCalledWith(false)
      expect(onChange).toHaveBeenLastCalledWith(null)
      // The typed text stays visible so the user can correct it
      expect(input).toHaveValue('30/02/2025')
    })

    test('accepts a valid typed date and reports it as valid', async () => {
      const user = userEvent.setup()
      const onChange = vi.fn()
      const onValidityChange = vi.fn()
