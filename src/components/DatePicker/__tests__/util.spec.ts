...
describe('isValidDatePatternString', () => {
  it('returns true for valid date-only string when hasTime=false', () => {
    expect(isValidDatePatternString('12032014', false)).toBe(true)
  })

  it('returns false for date-only string when hasTime=true', () => {
    expect(isValidDatePatternString('12032014', true)).toBe(false)
  })

  it('returns true for valid date+time string when hasTime=true', () => {
    expect(isValidDatePatternString('120320141130', true)).toBe(true)
  })

  it('returns false for date+time string when hasTime=false', () => {
    expect(isValidDatePatternString('120320141130', false)).toBe(false)
  })

  it('rejects invalid day/month combos', () => {
    expect(isValidDatePatternString('31022020', false)).toBe(false) // Feb 31
    expect(isValidDatePatternString('01132020', false)).toBe(false) // month 13
  })

  it('rejects invalid hour/minute', () => {
    expect(isValidDatePatternString('1203202475', true)).toBe(false) // hour 24, min 75
  })

  it('rejects wrong lengths', () => {
    expect(isValidDatePatternString('1203201', false)).toBe(false)
    expect(isValidDatePatternString('12032014123', true)).toBe(false)
  })
})
