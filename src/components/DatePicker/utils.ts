...
const MIN_DATEPICKER_HEIGHT = 600
const DATEPICKER_MARGIN_BOTTOM_OVERRIDE = '400px'
...
// Handle document scroll when calendar opens on short pages
export const useDocumentScroll = ({
  ...
  useEffect(() => {
    ...
    const handleDocumentScroll = () => {
      ...
      if (isCalendarOpen && documentHeight < MIN_DATEPICKER_HEIGHT) {
        datePicker.style.marginBottom = DATEPICKER_MARGIN_BOTTOM_OVERRIDE
      } else if (
        datePicker.style.marginBottom === DATEPICKER_MARGIN_BOTTOM_OVERRIDE
      ) {
        datePicker.style.marginBottom = ''
      }
    }
