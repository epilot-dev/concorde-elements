...
describe('Divider', () => {
  ...
  describe('label', () => {
    test('renders the label centered between two divider lines', () => {
      const { container } = render(<Divider label="or" />)

      const label = container.querySelector('.Concorde-Divider__label')

      expect(label).not.toBeNull()
      expect(label?.textContent).toBe('or')
      expect(container.querySelectorAll('hr')).toHaveLength(2)
      expect(label?.previousElementSibling?.tagName).toBe('HR')
      expect(label?.nextElementSibling?.tagName).toBe('HR')
    })

    test('renders a single line without a label', () => {
      const { container } = render(<Divider />)

      expect(container.querySelector('.Concorde-Divider__label')).toBeNull()
